...
public class LaneChangeModel {
    ...
    public LaneChangeDecision makeDecision(RoadSegment roadSegment) {
        ...
        double lane_change_signal = lcModelMOBIL.getParameter().getRightBiasAcceleration();
        if (lane_change_signal == Lanes.ENFORCE_STRAIGHT){
            return decision;
        }
        else if (lane_change_signal == Lanes.ENFORCE_LEFT_TURN || lane_change_signal == Lanes.ENFORCE_RIGHT_TURN){
            if (me.lane() == roadSegment.trafficLaneMax() || me.lane() == Lanes.MOST_INNER_LANE){
                return decision;
            }
        }
    ...
    private LaneChangeDecision determineDiscretionaryLaneChangeDirection(RoadSegment roadSegment) {
        ...
        if (currentLane + Lanes.TO_RIGHT <= roadSegment.trafficLaneMax()) {
            ...
            if (newLaneSegment.type() == Lanes.Type.TRAFFIC) {
                ...
                // changing
                if (lane_change_signal == Lanes.ENFORCE_RIGHT_TURN){
                    accToRight = Double.MAX_VALUE;
                }
                else {
        ...
        if (currentLane + Lanes.TO_LEFT >= Lanes.MOST_INNER_LANE) {
            ...
            if (newLaneSegment.type() == Lanes.Type.TRAFFIC) {
                // only consider lane changes into traffic lanes, other lane changes are handled by mandatory lane
                // changing
                if (lane_change_signal == Lanes.ENFORCE_LEFT_TURN){
                    accToLeft = Double.MAX_VALUE;
                }
                else {
